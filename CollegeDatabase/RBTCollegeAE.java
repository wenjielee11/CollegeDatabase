...
/**
 * This class implements the algorithm for a college database, including lookups for any specified field in a range without duplicate code.
 */
public class RBTCollegeAE extends RedBlackTreeAE<CollegeInterface> implements RBTCollegeInterface<CollegeInterface>{
    /**
     ...
     */
    RBTCollegeAE(String filter){
        super();
        this.filter = filter;
        try {
            field = CollegeDW.class.getField(filter);
    ...
    public CollegeInterface search(String filter, Comparable value) {
        ...
        Field f = null;
        try {
            f = CollegeDW.class.getField(filter);
        } catch (NoSuchFieldException e) {
            e.printStackTrace();
            return null;
        }
